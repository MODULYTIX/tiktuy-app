...
interface ButtonProps {
  ...
  disabled?: boolean;
  className?: string;
  /** NUEVO: posición del ícono (izquierda por defecto) */
...
const Buttonx: React.FC<ButtonProps> = ({
  ...
  // Variantes (solo corregí el texto blanco en quartery)
  const variantStyles: Record<ButtonVariant, string> = {
    ...
    quartery: "bg-gray90 text-white hover:bg-gray-300", // <- fix
    outlined: "border-2 bg-gray-50 font-medium border-gray-400 text-gray-500 hover:bg-gray-100",
    outlinedw: "border-[1.5px] font-medium border-gray80 text-black hover:bg-gray-100",
  };
  ...
  return (
    <button
      ...
      type="button"
    >
      {/* Render condicional según la posición */}
      {icon && iconPosition === "left" && <Icon icon={icon} width={22} height={22} aria-hidden="true" />}
      {label}
      {icon && iconPosition === "right" && <Icon icon={icon} width={22} height={22} aria-hidden="true" />}
    </button>
  );
};
