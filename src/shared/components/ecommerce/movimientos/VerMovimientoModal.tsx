    ...
    <div className="fixed inset-0 z-50 flex">
      ...
      <div className="w-full max-w-md bg-white shadow-lg h-full p-6 overflow-y-auto">
        <h2 className="text-xl font-bold flex items-center gap-2">
          <HiOutlineEye />
          DETALLE PARA MOVIMIENTO
        </h2>
        <p className="text-sm text-gray-500 mt-1">
          Consulta los datos del producto antes de registrar el movimiento.
        </p>
        ...
        <div className="mt-6 space-y-4 text-sm">
          ...
          <div className="grid grid-cols-2 gap-3">
            <Field
              label="Categoría"
              value={String((data as any).categoria?.descripcion ?? (data as any).categoria_id ?? '')}
            />
            <Field
              label="Almacén"
              value={String((data as any).almacenamiento?.nombre_almacen ?? (data as any).almacenamiento_id ?? '')}
            />
          </div>
          <div className="grid grid-cols-2 gap-3">
