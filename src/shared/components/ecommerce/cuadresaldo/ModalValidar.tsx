...
type Props = {
  ...
  ciudad?: string; // si quieres mostrar
...
export default function ValidarAbonoModal({
  ...
  const neto = useMemo(() => totalCobrado - totalServicio, [totalCobrado, totalServicio]);
      ...
      <div className="w-full max-w-md rounded-2xl bg-white shadow-xl">
        ...
        <div className="mx-6 my-4 rounded-xl border">
          <div className="grid grid-cols-2 gap-2 px-4 py-3 text-sm">
            <div className="font-semibold">{money(totalCobrado)}</div>
            ...
            <div className="text-right font-medium">{money(neto)}</div>
            <div className="text-gray-500">Estado</div>
            <div className="text-right"><span className="rounded-full bg-blue-100 px-2 py-0.5 text-xs text-blue-900 border border-blue-200">Por Validar</span></div>
          </div>
        </div>
