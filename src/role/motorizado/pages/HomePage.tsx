...
const isAbort = (e: unknown) => (e as any)?.name === 'AbortError' || /aborted/i.test((e as any)?.message || '');

export default function MotorizadoHomePage() {
  const { user, token } = useAuth();
  const [activo, setActivo] = useState<boolean | null>(null);
  const [toggling, setToggling] = useState(false);
  const [kpis, setKpis] = useState<KPIs>({ asignadosHoy: 0, completados: 0, pendientes: 0, reprogramados: 0 });
  ...
  const ctaSubtitle = useMemo(() => {
    if (kpis.pendientes > 0) return `Tienes ${kpis.pendientes} entregas pendientes hoy. Ingresa al módulo de pedidos para más detalles y acciones.`;
  ...
  const load = useCallback(
    async (signal?: AbortSignal) => {
      ...
      if (dispRes.status === 'fulfilled') setActivo(dispRes.value.activo);
    ...
    <div className="min-h-screen bg-gray-50">
      <header className="sticky top-0 z-10 border-b bg-white/80 backdrop-blur">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-3">
          ...
          <div className="flex items-center gap-3">
            <span className={`text-sm font-medium ${activo ? 'text-emerald-600' : 'text-gray-600'}`}>{estadoText}</span>
            <button type="button" onClick={onToggle} disabled={toggling || activo === null} aria-pressed={!!activo} className={`relative inline-flex h-7 w-14 items-center rounded-full transition ${activo ? 'bg-emerald-500' : 'bg-gray-300'}`}>
              <span className="sr-only">Cambiar disponibilidad</span>
              <span className={`inline-block h-6 w-6 transform rounded-full bg-white transition ${activo ? 'translate-x-7' : 'translate-x-1'}`} />
            </button>
          </div>
        </div>
      </header>
      ...
      <main className="mx-auto max-w-7xl px-4 py-6">
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <KpiCard title="Pedidos Asignados Hoy" value={kpis.asignadosHoy} icon="mdi:note-edit" accent="blue" disabled={!activo} />
          <KpiCard title="Entregas completadas" value={kpis.completados} icon="mdi:clipboard-check" accent="green" disabled={!activo} />
          <KpiCard title="Entregas Pendientes" value={kpis.pendientes} icon="mdi:clipboard-alert" accent="amber" disabled={!activo} />
          <KpiCard title="Pedidos Reprogramados" value={kpis.reprogramados} icon="mdi:calendar-refresh" accent="red" disabled={!activo} />
        </div>
          ...
          {loading ? (
            ...
            showCTA ? (
              <div className="rounded-2xl border bg-white p-6">
                ...
                <div className="mt-4">
                  <Link to={GESTION_PEDIDOS_PATH} className="inline-flex items-center rounded-xl bg-slate-900 px-4 py-2 text-white shadow-sm hover:bg-slate-800">
              ...
              <div className="flex flex-col items-center justify-center rounded-2xl border bg-white p-8 text-center text-gray-600">
                ...
                <p className="mt-4 max-w-[48ch] text-sm leading-6">Aún no tienes entregas asignadas. Te avisaremos apenas llegue una.</p>
              </div>
            )
          ) : (
            <div className="flex flex-col items-center justify-center rounded-2xl border bg-white p-8 text-center">
              ...
              <p className="mt-6 max-w-[48ch] text-sm leading-6 text-gray-600">Actualmente estás inactivo. Activa tu estado para recibir pedidos asignados por tu courier.</p>
            </div>
          )}
