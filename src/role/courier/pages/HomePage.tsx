...
export default function CourierHomePage() {
  ...
  return (
    <div className="mt-6 px-4">
      ...
      {/* Tabla dinámica */}
      {activeTab === 'ecommerce' ? <PanelControlTable /> : <PanelControlRepartidor />}

       {/* Modal Compartir */}
      {isModalOpen && (
        <PanelControlInvitacion onClose={() => setIsModalOpen(false)} activeTab={activeTab} />
      )}

      {/* Drawer de registro */}
      {isDrawerOpen && (
        <div className="fixed inset-0 bg-black/25 bg-opacity-30 z-50 flex justify-end">
          <div className="w-[450px] bg-white p-6 overflow-y-auto">
           

            {activeTab === 'ecommerce' ? (
              <PanelControlRegistroEcommerce onClose={() => setIsDrawerOpen(false)} />
            ) : (
              <PanelControlRegistroRepartidor onClose={() => setIsDrawerOpen(false)} />
            )}
          </div>
        </div>
      )}
